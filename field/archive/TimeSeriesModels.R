...
tilhill = read_excel('../../data/TilhillWorkingCopy.xlsx', sheet = 'GEEdata')

#look at quick summary
mean_heights <- tilhill %>%
...
ggplot(tilhill, aes(x = factor(time), y = height, fill = treatment)) +
  geom_boxplot() +
...
#linear model
mixed_model = mixed_model <- lmer(height ~ time * treatment + avg_baseline_height+ (1 | plot), data = data)
...
##############################################################################
####add a baseline height in as a covariate.
...
baseline_avg <- aggregate(height ~ treatment, data = data[data$time == 0, ], mean)
colnames(baseline_avg)[2] <- "avg_baseline_height"
data <- merge(data, baseline_avg, by = "treatment", all.x = TRUE)


#exponential model with baseline 
mixed_model_exp = mixed_model <- lmer(log(height) ~ time * treatment + avg_baseline_height +(1 | plot), data = data)
