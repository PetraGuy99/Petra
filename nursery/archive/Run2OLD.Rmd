...
This analysis summarize Elsoms run 2. Nursery Trials/Experiments/Run 1/Elsoms Run 2 Results

This is Run 2 at Elsoms - in this run we applied 6 different emf using culture media - not pellets. Note that the hyphal density in each application was not measured hence the amount of pax applied, say, could have been greater than the amount of laccaria. Equal volumes were applied to each plant, and the media agitated between each application, but the amount of culture within each flask varied. All the oaks had mildew which is likely to have stunted their growth
...
First look at Bench 2 - this is the one furthest from the door. 

Summary table of means and medians for oaks on Bench 2

```{r}
plot_data <- oak %>% filter(Drought == "Non", !is.na(Delta_H))
...
Box plots for change in height of OAK on Bench 2, with p values. Note that p values are for 2 tailed t.test with equal variances (not tested). This is taking each tree to be
an individual. It could be argued that the tray is the unit, but I would suggest that
we can use each tree since culture was applied individually, the cells are separate,
the trays were placed on the bench in groups to minimize variation due to light and watering - i.e. the oaks were all at the top end of the benches, birches in the middle and so on. It is probably likely that the most important factor for unknown variance is the genetics of the tree and perhaps the hyphal density of inoculum received, so it is probably acceptable to carry out a t test on each tray as a group.
...
# t-test p-values vs control
pvals <- lapply(fungi_order, function(f) {
  t_res <- t.test(Delta_H ~ Fungi,
...
Bench 1 - the bench by the door, which did receive some random droughting, cant be sure
how this was applied for different trays. 
...
Compare the differences for oaks on each bench.
...
There is a more significant change in height for the oaks on bench 1 compared to those on bench 2. Particularly for lactarius and scleroderma.

Since we have a lot of unplanned variation, from watering issues to density of media, perhaps we should combine all data across all oaks and treat as one big batch.
...
If we accept that t.test is valid, then we have significant changes in height for hebeloma, lactarius, pax and scleroderma, with the greatest height change due to lactarius.


A different bar plot. This is quite nice because it doesnt make any claims to p values or se etc - but shows the spread of points, so its totally transparent in terms of the actual raw data.
